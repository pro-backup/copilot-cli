...
1. **Workload IPv6 is auto-derived**, not a manifest opt-in. If
   ...
   is dual-stack.
2. **Env IPv6 state is read from the env manifest at stack-construction
   time.** Each workload stack's config already receives
   `EnvManifest *manifest.Environment`; the new accessor
   `envManifest.Network.VPC.IPv6Enabled()` (shipped by #1) is the source
   of truth. Since #1 blocks toggling via `errIPv6ToggleOnExistingEnv`,
   the manifest value and the deployed env-stack state cannot diverge.
   Service stacks do not use `Fn::ImportValue` on the env's IPv6
   outputs — an output that may be absent would otherwise break every
   v4-only service deploy.
...
At stack-construction time (`NewBackendService`, `NewLoadBalancedWebService`,
etc.), copilot reads `conf.EnvManifest.Network.VPC.IPv6Enabled()` and
stores the result on the stack struct. During `Template()`, that bool
is stamped onto the new `IPv6Enabled` field of `template.NetworkOpts`.
...
The service and job stack configs already carry
`EnvManifest *manifest.Environment`. At construction time
(`NewBackendService`, `NewLoadBalancedWebService`,
`NewWorkerService`, `NewScheduledJob`), each stack reads
`conf.EnvManifest.Network.VPC.IPv6Enabled()` once and stores the
result on a new `envIPv6Enabled bool` field on the stack struct. This
mirrors the existing pattern where `httpsEnabled`, `albEnabled`, and
similar flags are precomputed on the stack struct during construction.

`convertNetworkConfig` gains a second parameter — a bool that gets
stamped onto `template.NetworkOpts.IPv6Enabled`. Every call site
passes the precomputed flag.

```go
func convertNetworkConfig(network manifest.NetworkConfig, ipv6Enabled bool) template.NetworkOpts {
    // ... existing logic unchanged ...
    opts.IPv6Enabled = ipv6Enabled
    return opts
}
```

No env-stack describe, no new mocks, no new interfaces. Because #1's
`errIPv6ToggleOnExistingEnv` blocks toggling, the env manifest's
`IPv6Enabled()` value and the deployed env-stack state cannot diverge.

The RDWS path (`convertRDWSNetworkConfig`) is App Runner, not ECS,
and is out of scope — its signature is untouched.
...
| `internal/pkg/deploy/cloudformation/stack/transformers.go` | `convertNetworkConfig` gains `ipv6Enabled bool` param | +5 |
| `internal/pkg/deploy/cloudformation/stack/lb_web_svc.go`, `backend_svc.go`, `worker_svc.go`, `scheduled_job.go` | `envIPv6Enabled bool` field on stack struct; populate in constructor; pass to `convertNetworkConfig` | +40 (all four) |
...
- `convertNetworkConfig(cfg, true)` sets `NetworkOpts.IPv6Enabled = true`.
- `convertNetworkConfig(cfg, false)` leaves `NetworkOpts.IPv6Enabled = false`.
- Each stack constructor populates `envIPv6Enabled` correctly from `conf.EnvManifest.Network.VPC.IPv6Enabled()` (covered by the integration goldens below).
...
- If a post-release bug surfaces, a patch can disable the feature by
  ...
  `envIPv6Enabled` field. Both are two-line changes.
